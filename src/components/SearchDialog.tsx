import { CheckCircle2, Folder, Search, X } from 'lucide-react'
...
export function SearchDialog() {
  const { searchOpen, setSearchOpen, setView, setProject } = useUIStore()
  const { tasks, projects } = useNavigationData()
  const [query, setQuery] = useState('')
  const deferredQuery = useDeferredValue(query.trim().toLowerCase())
  const inputRef = useRef<HTMLInputElement>(null)
  const projectMap = useMemo(() => new Map(projects.map((project) => [project.id, project])), [projects])
  const results = useMemo(() => {
    if (!deferredQuery) return tasks.slice(0, 8)
  ...
  }, [deferredQuery, projectMap, tasks])
  ...
  return (
    <div className="overlay-layer" role="dialog" aria-modal="true" aria-labelledby="search-title" onClick={() => setSearchOpen(false)}>
      ...
      <section className="search-dialog" onClick={(event) => event.stopPropagation()}>
        <header>
          <Search size={19} />
          <input ref={inputRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search to-dos, notes, tags, and projects" aria-label="Search Daymark" />
        ...
        <div className="search-results">
          ...
          }) : <p className="search-results__empty">No matching to-dos.</p>}
        </div>
      </section>
    </div>
  )
